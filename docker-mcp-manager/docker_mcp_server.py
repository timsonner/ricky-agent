...
def _safe_join(base_path: str, requested_path: str) -> str:
    """
    Safely join base path and requested path, preventing directory traversal.

    Args:
        base_path: The base directory that paths must be restricted to
        requested_path: The user-provided path (will be treated as relative to base_path)
    ...
    # Treat the requested path as relative to the base path, regardless of leading slashes
    # This ensures all file operations are confined to the base directory
    if requested_path.startswith("/"):
        # Remove leading slash to treat as relative path
        requested_path = requested_path.lstrip("/")

    # Handle empty or dot paths
...
def _get_container_info() -> Dict[str, Any]:
    ...
    try:
        ...
        result = run_docker_command(["docker", "inspect", CONTAINER_NAME])
        ...
        if result.returncode != 0:
            if (
                ...
                raise Exception(
                    f"Container {CONTAINER_NAME} not found. Is docker-compose up?"
                )
            else:
        ...
        if state.get("Status") != "running":
            raise Exception(
                f"Container {CONTAINER_NAME} is not running (status: {state.get('Status')})"
            )
...
async def execute_command(command: str) -> str:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning(
                f"Command exited with code {result.returncode}: {output_str}"
            )
            return f"Command failed with exit code {result.returncode}:\n{output_str}"
        else:
            logger.info(
                f"Command executed successfully: {output_str[:100]}{'...' if len(output_str) > 100 else ''}"
            )
...
async def read_file(file_path: str) -> str:
    ...
    try:
        ...
        full_path = _safe_join(PERSISTENT_MOUNT_POINT, file_path)
    ...
    try:
        ...
        result = run_docker_command(
            [
                ...
                "sh",
                "-c",
                f'printf %s "{escaped_content}" > {full_path}',
            ]
        )
...
async def list_directory(dir_path: str) -> List[str]:
    ...
    try:
        # Safely join the path to prevent directory traversal
        full_path = _safe_join(PERSISTENT_MOUNT_POINT, dir_path)
    except ValueError as e:
        logger.error(f"Path validation failed: {e}")
        return [f"Error: {str(e)}"]
    ...
    try:
        ...
        result = run_docker_command(
            ["docker", "exec", CONTAINER_NAME, "ls", "-la", full_path]
        )
...
async def get_container_info() -> Dict[str, Any]:
    ...
    try:
        ...
        container_info = (
            _get_container_info()
        )  # This will use the function we defined earlier
    ...
    try:
        ...
        result = run_docker_command(
            [
                ...
                "sh",
                "-c",
                f"test -e {full_path} && echo 'exists' || echo 'not found'",
            ]
        )
